...
!!!!!(weitingc, 2021/03/15)
!!!!!將module分成三個level，選題也從三個level中選出。
program MST
    implicit none
    ...
    real::infor_sum(n_moduleinpool) !n_module=20
    integer::place_infor_sum(n_moduleinpool) 
    real::randv 
    real::randv_resp(row,length) 
    
    
    integer :: used(col)=0
    real::theta_begin(row)=0
    real::theta_hat(row,stage)=0
    integer::resp(row,length)=99
    integer::place_choose(row,length)
    
    !integer :: countA(col)=0, poolused(col)=0
    
    integer :: i,j,try,choose,k
    ...
    integer:: countA(col)=(/(0,i=1,col)/) !使用
    integer::countPOOL(col)=(/(0,i=1,col)/) !題庫使用
    !real:: PS(col)=(/(0,i=1,col)/) !選中 Select
    real:: PA(col)=(/(0,i=1,col)/) !使用 Administer
    real:: PK(col)=(/(1,i=1,col)/)
    real:: maxPA
    integer::place_maxPA
    integer::sumS !檢驗用
    integer::sumA !檢驗用
    ...
    open(100,file="parameter_MST.txt",status="old") 
    do i=1,col
    ...
    do try=1,row
        ...
        j=1
        infor_sum(1)=infor(1)
        place_infor_sum(1)=place_infor(1)
        do i=2, col
            if (place_infor(i)==place_infor(i-1)) then		
                infor_sum(j)=infor_sum(j)+infor(i)
            else
                j=j+1
                infor_sum(j)=infor(i)
                place_infor_sum(j)=place_infor(i)
            endif	
        end do
        ...
        place=0
        i=0
        do i=1,n_moduleinpool
            if (infor_sum(i)>maxv)then
                maxv=infor_sum(i)
                place=place_infor_sum(i)
            endif
        enddo
        ...
        j=0
        do i=1,col
            if (modulev(i)==place) then
                j=j+1
                
                used(i)=1
                place_choose(try,j)=i
                a_choose(try,j)=a(i) !存入選題
                b_choose(try,j)=b(i)
                c_choose(try,j)=c(i)
                ...
                a_choose(try,j),b_choose(try,j),c_choose(try,j),&
                resp(try,j),randv)
                randv_resp(try,j)=randv
    
    
            else
                countA(i)=countA(i)
                countPOOL(i)=countPOOL(i)
                !print*, j
            end if
        end do
        
        choose = stage_n * n_item
        call EAP(a_choose,b_choose,c_choose,resp,row,length,theta_hat,try,choose,stage,stage_n)
        !print*, choose
        !print*, theta_hat(try,stage_n)
        ...
        do i=1,col
            ...
            if (used(i)==1) then
                ...
                infor(i)=information(theta_hat(try,stage_n-1),a(i),b(i),c(i))
            end if
        end do
        !print*, infor	
    ...
    OmegaGA3_Min = 1
    do i=1,row !10000
        thetaHATmean = thetaHATmean + theta_hat(i,stage)
    ...
    OmegaGA3_Variance = 0
    do i=1,row !10000
        thetaHatVariance = thetaHatVariance + (theta_hat(i,stage)-thetaHATmean)**2
        thetaHatMSE = thetaHatMSE + (theta_hat(i,stage) - thetaTRUEmean)**2
    ...
    !------------------
    open(101,file="summery_MST_2_thetaHAT.txt",status="replace") !寫下能力估計
    open(102,file="summery_MST_2_response.txt",status="replace") !寫下作答反應
    open(103,file="summery_MST_2_item.txt",status="replace") !寫下所選的試題
    open(104,file="summery_MST_2_m.txt",status="replace") !紀錄m
    open(105,file="summery_MST_2_Omega.txt",status="replace")
    open(106,file="summery_MST_2_randv.txt",status="replace")
    
    write(101,"(A5,A10,20I10)") "ID","true",(i,i=1,stage)
    ...
    write(106,"(A5,50I10)") "ID",(i,i=1,length)
    
    do i=1,row !10000
        write(101,"(I5,20F10.4)") i,thetaTRUE(i),(theta_hat(i,j),j=1,stage)
        ...
        write(106,"(I5,50F10.4)") i,(randv_resp(i,j),j=1,length) !寫下所選的試題
        !write(*,*) i,(resp(i,j),j=1,length)
    end do
    
    !寫入所需資料
    open(331,file="summery_MST_2_summary.txt",status="replace") 
    ...
    !EAP能力估計
    !------------------
    subroutine EAP(a,b,c,resp,row,length,thetaHAT,try,choose,stage,stage_n)
    implicit none
    integer::i,j,row,length,stage,stage_n
    integer::try,choose
    real::a(row,length),b(row,length),c(row,length) !選題的試題參數
    integer::resp(row,length) !作答反應
    real::thetaHAT(row,stage) !估計能力值
    ...
    thetaHAT(try,stage_n)=EAP_numerator/EAP_denominator !計算出能力估計值
