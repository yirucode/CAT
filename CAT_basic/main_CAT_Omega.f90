program CAT
    ...
    do try = 1, numTest
        ...
        do  choose = 1, length
            ...
            if (try <= alpha) then 
                ...
                if ( choose == 1 ) then
                    ! 設定Psi控制
                    deltaCriteria(choose, try) = func_deltaPsi(try,alpha,omegaMax,choose)
                    do i = 1, numPool
            ...
            if ( try > alpha ) then 
                if ( choose+1 <= length ) then
                    deltaCriteria(choose+1, try) = func_deltaPsi(try,alpha,omegaMax,1)
                    ! deltaCriteria(choose+1, try) = func_deltaPsi(try,alpha,omegaMax,choose+1)
    ...
    write(unit = 100, fmt = '(A10,F10.5)') "psi max = ", omegaMax ! 增加Psi的設定指標
