program MST_1234_design
    implicit none
    ! === given data ====
    ! === 輸入資料設定 ===
    character(len = 50), parameter :: dataPath = "data/parameter_MST_1-2-3-4_data_P.txt"
    character(len = 50), parameter :: dataPath2 = "data/Population_Uniform.txt"!Population_Uniform.txt" !Normal_Population.txt
    ! === MST set ===
    integer,parameter :: numStages = 4
    integer :: numModulInStage(numStages) = (/1,2,3,4/)
    integer,parameter :: maxModule = 20 ! 1*4+2*3+3*2+4*1
    integer, parameter :: numItemInModule = 10
    ! === parameter ===
    integer,parameter :: numTest = 10000 !重複次數
    integer,parameter :: numPool = 200 !題庫數
    ...
    ! === choose module ===
    real :: rand_module(numStages, numTest) !使用於選Modules
    ! === MST 的運算暫存 ===
    ...
    do try = 1,numTest
        do  choose = 1, numStages
            ...
            if (choose == 1) then
                placeModule_choose(choose, try) = INT(1+FLOOR(4*rand_module(choose,try)))
            else if (choose == 2) then
                if (thetaHat(choose-1, try)>thetaBegin) then
                    placeModule_choose(choose, try) = INT(4+1+FLOOR(3*rand_module(choose,try)))
                else
                    placeModule_choose(choose, try) = INT(7+1+FLOOR(3*rand_module(choose,try)))
                endif
            else if (choose == 3) then
                if (thetaHat(choose-2, try)>thetaBegin) then
                    if(thetaHat(choose-1, try)>thetaHat(choose-2, try)) then
                        placeModule_choose(choose, try) = INT(10+1+FLOOR(2*rand_module(choose,try)))
                    else
                        placeModule_choose(choose, try) = INT(12+1+FLOOR(2*rand_module(choose,try)))
                    endif
                else
                    if(thetaHat(choose-1, try)>thetaHat(choose-2, try)) then
                        placeModule_choose(choose, try) = INT(12+1+FLOOR(2*rand_module(choose,try)))
                    else
                        placeModule_choose(choose, try) = INT(14+1+FLOOR(2*rand_module(choose,try)))
                    endif
                endif
            else if (choose == 4) then   
                if ((thetaHat(choose-3, try)>thetaBegin) .AND. (thetaHat(choose-2, try)>thetaHat(choose-3, try))) then
                    if(thetaHat(choose-1, try)>thetaHat(choose-2, try)) then
                        placeModule_choose(choose, try) = INT(17)
                    else
                        placeModule_choose(choose, try) = INT(18)
                    endif
                else if ((thetaHat(choose-3, try)>thetaBegin) .AND. (thetaHat(choose-2, try)<thetaHat(choose-3, try))) then
                    if(thetaHat(choose-1, try)>thetaHat(choose-2, try)) then
                        placeModule_choose(choose, try) = INT(18)
                    else
                        placeModule_choose(choose, try) = INT(19)
                    endif
                else if ((thetaHat(choose-3, try)<thetaBegin) .AND. (thetaHat(choose-2, try)>thetaHat(choose-3, try))) then
                    if(thetaHat(choose-1, try)>thetaHat(choose-2, try)) then
                        placeModule_choose(choose, try) = INT(18)
                    ...
                    if(thetaHat(choose-1, try)>thetaHat(choose-2, try)) then
                        ...
                        placeModule_choose(choose, try) = INT(20)
                    endif
                endif
            endif
            ...
            if (choose == 1) then 
                ...
                if (thetaHat(choose-1, try) > thetaBegin) then
                    placeModule_choose(choose, try) = placeModule_choose(choose-1, try) + numModulInStage(choose-1)
                else if (thetaHat(choose-1, try) <= thetaBegin) then
                    placeModule_choose(choose, try) = placeModule_choose(choose-1, try) + numModulInStage(choose-1) +1 
                endif
            else
                if (thetaHat(choose-1, try) > thetaHat(choose-2, try)) then
                    placeModule_choose(choose, try) = placeModule_choose(choose-1, try) + numModulInStage(choose-1)
                else if (thetaHat(choose-1, try) <= thetaHat(choose-2, try)) then
                    placeModule_choose(choose, try) = placeModule_choose(choose-1, try) + numModulInStage(choose-1) +1 
                endif
            endif            
