...
!EAP能力估計
!------------------
subroutine EAP(row,length,try,choose,a,b,c,resp,thetaHAT)
    implicit none
    ! === output data ===
    real, intent(out)::thetaHAT(row) !估計能力值
    ! === input data ===
    integer, intent(in)::row, length ! 總人數；作答題長
    integer, intent(in)::try, choose ! 第try人作答的第choose題
    !integer, intent(in)::stage, stage_n ! 總階數；第幾階
    integer, intent(in)::resp(row,length) !作答反應
    real, intent(in)::a(row,length),b(row,length),c(row,length) !選題的試題參數
    ! === function ===
    ...
    thetaHAT(try)=EAP_numerator/EAP_denominator !計算出能力估計值
    return
end subroutine EAP
...
program ex
    ...
    WRITE(*,*) 'sumB=',sumv
    k = 5 ! 只針對前5題算EAP 
    call EAP(row,length,try,choose,a,b,c,resp,thetaHAT)
